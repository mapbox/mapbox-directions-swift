...
if [ $# -ne 3 ]; then
    ...
    echo "Example: $0 2.14.3 docs.mapbox.com-staging build/docs/2.14.3"
    exit 1
fi
...
s5cmd sync \
    "${SOURCE_PATH}/*" \
    "s3://${TARGET_ENVIRONMENT}/ios/directions/api/${VERSION}/"

echo "########################################################"
echo "DOCS PUBLISHED SUCCESSFULLY"
echo "URL: https://${TARGET_ENVIRONMENT}/ios/directions/api/${VERSION}/"
echo "########################################################"
