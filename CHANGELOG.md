...
## v2.14.4
* Duplicates of URL request parameters derived from `DirectionsOptions`, `IsochroneOptions` and `MatrixOptions` are now removed when constructing a request URL, and an error is logged when duplicates are detected.

## v2.14.3

* Fixed an issue where authentication route request parameters could be duplicated when using a custom `DirectionsOptions` subclass. ([#840](https://github.com/mapbox/mapbox-directions-swift/pull/840))
