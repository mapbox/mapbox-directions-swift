...
extension Route {
     ...
     */
    public mutating func refresh(from refreshedRoute: RouteRefreshSource, refreshParameters: RefreshParameters = RefreshParameters()) {
     ...
     */
    public mutating func refreshLegAttributes(from refreshedRoute: RouteRefreshSource) {
        var legIndex = legs.startIndex
        for refreshedLeg in refreshedRoute.refreshedLegs {
            guard legIndex < legs.endIndex else { break }
            legs[legIndex].attributes = refreshedLeg.refreshedAttributes
            legIndex = legs.index(after: legIndex)
        }
    }
     ...
     */
    public mutating func refreshLegAttributes(from refreshedRoute: RouteRefreshSource, legIndex: Int, legShapeIndex: Int) {
        guard legIndex + refreshedRoute.refreshedLegs.count <= legs.count else { return }
        var legIndex = legIndex
        for refreshedLeg in refreshedRoute.refreshedLegs {
            guard legIndex < legs.endIndex else { break }

            let startIndex = legIndex == 0 ? legShapeIndex : 0
            legs[legIndex].refreshAttributes(newAttributes: refreshedLeg.refreshedAttributes,
     ...
     */
    public mutating func refreshLegIncidents(from refreshedRoute: RouteRefreshSource) {
        var legIndex = legs.startIndex
        for refreshedLeg in refreshedRoute.refreshedLegs {
            guard legIndex < legs.endIndex else { break }
            legs[legIndex].incidents = refreshedLeg.refreshedIncidents
            legIndex = legs.index(after: legIndex)
        }
    }
    ...
    /**
     ...
     - parameter legShapeIndex: Index of a geometry of the `legIndex` leg, where to start refreshing from.
     */
    public mutating func refreshLegIncidents(from refreshedRoute: RouteRefreshSource, legIndex: Int, legShapeIndex: Int) {
        ...
        for index in legs.indices {
            if (legIndex..<endRefreshIndex).contains(index) {
                ...
                legs[index].refreshIncidents(newIncidents: refreshedLeg.refreshedIncidents,
                                             startLegShapeIndex: startIndex)
            } else {
                legs[index].incidents = nil
            }
        }
    }
    ...
    /**
     ...
     - parameter legShapeIndex: Index of a geometry of the `legIndex` leg, where to start refreshing from.
     */
    public mutating func refreshLegClosures(from refreshedRoute: RouteRefreshSource, legIndex: Int = 0, legShapeIndex: Int = 0) {
        let endRefreshIndex = legIndex + refreshedRoute.refreshedLegs.count
        for index in legs.indices {
            if (legIndex..<endRefreshIndex).contains(index) {
                let refreshedLeg = refreshedRoute.refreshedLegs[index-legIndex]
                let startIndex = index == legIndex ? legShapeIndex : 0
                legs[index].refreshClosures(newClosures: refreshedLeg.refreshedClosures,
                                            startLegShapeIndex: startIndex)
            } else {
                legs[index].closures = nil
            }
        }
    }
}
